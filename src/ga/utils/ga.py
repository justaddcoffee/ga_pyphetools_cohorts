...
def initialize_profiles(
    ...
    return pd.DataFrame(
        new_profiles, columns=["profile_id", "hpo_term_id", "weight", "negated"]
    )


def compare_profiles_to_patients(
    semsimian, pt_train_df: pd.DataFrame, profiles_pd: pd.DataFrame, debug=False
):
    ...
    result["similarity"] = result.apply(
        lambda row: compare_person_and_profile(row["person_id"], row["profile_id"]),
        axis=1,
    )
    return result
...
def move_terms_on_hierarchy(
    profiles, move_term_p, hpo_graph, include_list=None, debug=False
):
    """Randomly move a term to parent or child on the hierarchy with a frequency of
    move_term_p
    """

    def move_term_on_hierarchy(
        term, include_list=include_list, hpo_graph=hpo_graph, debug=debug
    ):
        if random.random() < move_term_p:
            ...
            if candidate_terms:
                ...
                if debug:
                    warnings.warn(
                        f"Didn't find any candidate parents or children for {term}"
                    )
                return term
        else:
            return term
...
def make_auc_df(
    sim_results: pd.DataFrame, patient_labels: pd.DataFrame
) -> pd.DataFrame:
    sim_results_with_labels = sim_results.merge(
        patient_labels, on="person_id", how="inner"
    )
...
def remove_terms_from_profiles_pd(
    profiles: pd.DataFrame, remove_term_p: float = 0.1
) -> pd.DataFrame:
...
def add_terms_to_profiles_pd(
        ...
        if random.random() < add_term_p:
            ...
            if hpo_term not in df["hpo_term_id"].unique():
                new_row = {
                    ...
                    "negated": True
                    if random.random() < fraction_negated_terms
                    else False,
    ...
    profiles = (
        profiles.reset_index(drop=True, inplace=False)
        .groupby("profile_id")
        .apply(add_random_hpo_terms)
    )

    return profiles


def change_weights_for_profiles(
    profiles: pd.DataFrame, change_weight_p: float = 0.1
) -> pd.DataFrame:
...
def recombine_profiles_pd(
    profiles: pd.DataFrame, ancestors_df: pd.DataFrame, num_profiles: int
) -> pd.DataFrame:
    """
    ...
    * return a new dataframe with num_profiles new profiles comprised of recombined profiles from parent dataframe
    ...
    parent_profiles = pd.DataFrame(
        list(itertools.product(all_pids, all_pids)), columns=["p1", "p2"]
    )
    parent_profiles = parent_profiles.loc[
        parent_profiles["p1"] < parent_profiles["p2"]
    ].sample(n=num_profiles // 2, replace=True)

    # Loop through pairs of profiles, and apply the recombination logic to each pair
    new_profiles_to_add = []
    for i, (profile1_id, profile2_id) in enumerate(
        parent_profiles[["p1", "p2"]].values
    ):
        profile1 = joined_profiles_df[
            joined_profiles_df.profile_id == profile1_id
        ].reset_index(drop=True)
        profile2 = joined_profiles_df[
            joined_profiles_df.profile_id == profile2_id
        ].reset_index(drop=True)
        ...
        random_hpo_term = (
            ancestors_df["hpo_term_id"].sample(n=1, random_state=1).iloc[0]
        )

        tmp_rows = []
        for _, row in profile1.iterrows():
            if (
                len(pd.isnull(row["ancestors"])) > 0
                and not pd.isnull(row["hpo_term_id"])
                and random_hpo_term in row["ancestors"]
            ):
                # This row['HPO_term'] is a descendant of random_hpo_term, do the swap

                # Create new row for profile2 with swapped HPO term
                new_row = profile1.loc[
                    profile1["hpo_term_id"] == row["hpo_term_id"]
        ...
        for _, row in profile2.iterrows():
            if (
                ...
                try:
                    rows_to_add_to_profile1.append(
                        pd.DataFrame(
                            [[new_profile1_id, row["hpo_term_id"], row["weight"]]],
                ...
                # catch ValueError
                    ...
                    print(
                        "ValueError: {} while adding row['hpo_term_id'] {} and row['weight'] {} to profile1".format(
                            ve, row["hpo_term_id"], row["weight"]
                        )
                    )

        # Add rows to profile1
        if rows_to_add_to_profile1:
            rows_to_add_to_profile1 = pd.concat(
                rows_to_add_to_profile1, ignore_index=True
            )
